...
public class Service extends com.alibaba.nacos.api.naming.pojo.Service implements Record, RecordListener<Instances> {
    ...
    /**
     * 这个方法里使用了CopyOnWrite思想，为了防止读写并发冲突
     * 具体的做法就是把原内存结构复制一份，操作完最后在替换回真正的注册表内存里去
     *
     * 而 Eureka 防止读写并发冲突用的方法是注册表的多级缓存结构，只读缓存，读写缓存，
     * 内存注册表各级缓存之间定时同步，客户端感知的及时性不如Nacos
     * @param instances
     * @param ephemeral
     */
    public void updateIPs(Collection<Instance> instances, boolean ephemeral) {
        ...
        for (Map.Entry<String, List<Instance>> entry : ipMap.entrySet()) {
            ...
            //将临时的注册实例更新到了cluster的ephemerallnstances属性上去，服务发现查找临时实例最终从内存里找到的就是这个属性
